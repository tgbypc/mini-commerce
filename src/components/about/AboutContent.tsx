    ...
    <div className="bg-[#f6f7fb] px-4 py-10 md:py-16">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-12">
        <section className="relative overflow-hidden rounded-4xl border border-zinc-200 bg-white/95 px-6 py-12 shadow-[0_28px_56px_rgba(15,23,42,0.1)] sm:px-10">
          <div
            className="absolute -right-24 top-0 size-80 rounded-full bg-gradient-to-br from-[#e3ecff] via-white to-[#f5f7ff] blur-3xl"
            aria-hidden
          />
          <div className="relative z-[1] space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {metrics.map((metric) => (
                <div
                  key={metric.label}
                  className="rounded-3xl border border-zinc-200 bg-[#f6f7fb] px-4 py-5 text-center shadow-sm"
                >
          ...
          <div className="space-y-4 rounded-4xl border border-zinc-200 bg-white/95 px-6 py-6 shadow-[0_20px_40px_rgba(15,23,42,0.08)]">
            ...
            <ol className="space-y-4 text-sm text-zinc-600">
              {timelineKeys.map((key) => (
                <li
                  key={key}
                  className="rounded-2xl border border-zinc-200/70 bg-[#f6f7fb] px-4 py-3"
                >
        ...
        <section aria-labelledby="about-values" className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {valueKeys.map((key) => (
              <div
                key={key}
                className="rounded-3xl border border-zinc-200 bg-white/95 px-5 py-5 shadow-sm"
              >
        ...
        <section aria-labelledby="about-team" className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {teamKeys.map((key) => (
              <div
                key={key}
                className="flex flex-col gap-3 rounded-3xl border border-zinc-200 bg-white/95 px-5 py-5 shadow-sm"
              >
        ...
        <section className="relative overflow-hidden rounded-4xl border border-zinc-200 bg-[var(--color-primary-dark)] px-6 py-10 text-white shadow-[0_28px_56px_rgba(15,23,42,0.35)] sm:px-10">
          <div
            className="absolute -left-24 top-0 size-72 rounded-full bg-white/15 blur-3xl"
            aria-hidden
          />
