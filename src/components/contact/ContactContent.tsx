    ...
    <div className="bg-[#f6f7fb] px-4 py-10 md:py-16">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-10">
        <section className="relative overflow-hidden rounded-4xl border border-zinc-200 bg-white/95 px-6 py-10 shadow-[0_24px_48px_rgba(15,23,42,0.08)] sm:px-10">
          <div className="absolute -right-16 top-[-30%] size-80 rounded-full bg-gradient-to-br from-[#ffeef8] via-white to-[#e6f1ff] blur-3xl" aria-hidden />
          <div className="relative z-[1] space-y-4">
            ...
            <h1 className="text-3xl font-semibold text-[#0d141c] md:text-4xl">{t('contact.hero.title')}</h1>
            <p className="max-w-3xl text-sm text-zinc-600 md:text-base">{t('contact.hero.subtitle')}</p>
            <div className="grid gap-4 sm:grid-cols-3">
              {['support', 'sales', 'press'].map((key) => (
                <div key={key} className="surface-card bg-white/95 px-4 py-4 text-sm text-[#0d141c]">
                  <div className="text-xs font-semibold uppercase tracking-[0.23em] text-zinc-500">
                    {t(`contact.cards.${key}.title`)}
                  </div>
                  <p className="mt-1 text-zinc-600">{t(`contact.cards.${key}.description`)}</p>
                  <Link
        ...
        <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 surface-card px-6 py-6 sm:px-8">
            <div className="space-y-2">
              <h2 className="text-xl font-semibold text-[#0d141c]">{t('contact.form.title')}</h2>
              <p className="text-sm text-zinc-600">{t('contact.form.description')}</p>
            </div>
            ...
            <button type="submit" disabled={submitting} className="mt-2 btn-primary disabled:cursor-not-allowed disabled:opacity-70">
          ...
          <div className="flex flex-col gap-6">
            <div className="surface-card px-6 py-6 sm:px-8">
              <h2 className="text-xl font-semibold text-[#0d141c]">{t('contact.locations.title')}</h2>
              <div className="mt-4 space-y-4 text-sm text-zinc-600">
                {locationKeys.map((key) => (
                  <div key={key} className="surface-card bg-white px-4 py-4">
                    <div className="text-sm font-semibold text-[#0d141c]">{t(`contact.locations.items.${key}.name`)}</div>
            ...
            <div id="faq" className="surface-card px-6 py-6 sm:px-8">
              <h2 className="text-xl font-semibold text-[#0d141c]">{t('contact.faq.title')}</h2>
              <div className="mt-4 space-y-4 text-sm text-zinc-600">
                {faqKeys.map((key) => (
                  <details key={key} className="rounded-3xl border border-[#5b5bd6]/20 bg-white px-4 py-3">
                    <summary className="cursor-pointer text-sm font-semibold text-[#0d141c]">
                      {t(`contact.faq.items.${key}.question`)}
                    </summary>
                    <p className="mt-2 text-sm text-zinc-600">{t(`contact.faq.items.${key}.answer`)}</p>
                  </details>
                ))}
              </div>
            </div>
