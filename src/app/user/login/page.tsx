...
export default function LoginPage() {
  ...
  const resolveRole = async (input: User | null) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resolvedRole = await resolveRole(credential.user)
      const destination = nextParam
        ? nextParam
        : resolvedRole === 'admin'
          ? '/admin'
    ...
    <div className="mx-auto grid max-w-4xl gap-10 px-4 py-16 sm:px-6 lg:px-8 lg:py-20 xl:grid-cols-[1.1fr_1fr]">
      <div className="hidden flex-col justify-between rounded-3xl border border-zinc-200/60 bg-white px-8 py-10 text-[#0d141c] shadow-[0_40px_80px_-40px_rgba(15,23,42,0.28)] dark:border-white/10 dark:bg-[rgba(24,32,45,0.75)] dark:text-white/90 dark:shadow-[0_40px_90px_-45px_rgba(13,18,27,0.65)] lg:flex">
        <div className="space-y-6">
          <span className="inline-flex items-center rounded-full border border-fuchsia-100 bg-fuchsia-50/70 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-fuchsia-500 dark:border-fuchsia-400/25 dark:bg-fuchsia-400/20 dark:text-fuchsia-100">
            {t('auth.login.kicker')}
          </span>
        ...
        <div className="space-y-4 text-sm text-zinc-500 dark:text-zinc-300/80">
          <div className="flex items-start gap-3">
            <span className="mt-1 size-2 rounded-full bg-indigo-400" aria-hidden />
            <span>{t('auth.login.benefits.one')}</span>
          </div>
          <div className="flex items-start gap-3">
            <span className="mt-1 size-2 rounded-full bg-indigo-400" aria-hidden />
            <span>{t('auth.login.benefits.two')}</span>
          </div>
          <div className="flex items-start gap-3">
            <span className="mt-1 size-2 rounded-full bg-indigo-400" aria-hidden />
            <span>{t('auth.login.benefits.three')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-zinc-200/60 bg-white/95 p-8 shadow-[0_30px_70px_-45px_rgba(15,23,42,0.35)] backdrop-blur dark:border-white/10 dark:bg-[rgba(15,18,28,0.92)] dark:shadow-[0_34px_70px_-45px_rgba(5,7,11,0.6)]">
        ...
        <div className="mt-6">
          ...
          >
            ...
            {socialLoading ? t('auth.login.googleLoading') : t('auth.login.googleCta')}
          </button>
        </div>
        <p className="mt-6 text-sm text-zinc-500 dark:text-zinc-300/80">
          {t('auth.login.footer')}{' '}
          <a href="/user/register" className="font-medium text-indigo-500 transition hover:text-indigo-600 dark:text-indigo-300 dark:hover:text-indigo-200">
