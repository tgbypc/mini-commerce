...
export default function ProfilePage() {
  ...
  async function saveAddress(e: React.FormEvent) {
    ...
    try {
      ...
      setAddresses(parseAddressList(items))
      toast.success(payload.id ? t('profile.addresses.toast.updated') : t('profile.addresses.toast.saved'))
      setAddrForm(createEmptyAddressForm())
      setShowAddressForm(false)
    } catch (err) {
      const message = err instanceof Error && err.message ? err.message : t('profile.addresses.toast.error')
      toast.error(message)
    } finally {
      setAddrSaving(false)
    }
  ...
  async function deleteAddress(id: string) {
    ...
    if (typeof window !== 'undefined') {
      const confirmDelete = window.confirm(t('profile.addresses.actions.confirmDelete'))
  ...
  const heroName = user?.displayName?.trim() || profileForm.displayName?.trim() || t('profile.hero.fallbackName')
  const heroGreeting = t('profile.hero.greeting').replace('{name}', heroName)
  const heroSubtitle = t('profile.hero.subtitle')
  const ordersCountLabel = t('profile.hero.ordersCount').replace('{count}', String(totalOrders))
  ...
  const resendLabel = verifySending
    ...
    : verifyCooldown > 0
      ? t('profile.verification.resendCooldown').replace('{seconds}', String(verifyCooldown))
    ...
    return (
      <div className="min-h-[60vh] bg-[#f6f7fb] px-4 py-12">
        <div className="mx-auto flex w-full max-w-3xl flex-col items-center gap-6 rounded-3xl border border-zinc-200 bg-white/90 px-8 py-14 text-center shadow-[0_18px_36px_rgba(15,23,42,0.08)]">
          <h1 className="text-3xl font-semibold text-[#0d141c]">{t('profile.title')}</h1>
          <p className="text-sm text-zinc-600 md:text-base">{t('profile.needLogin')}</p>
          <Link
            href="/user/login"
    ...
    <div className="min-h-screen bg-[#f6f7fb] px-4 py-10 md:py-12">
      <div className="mx-auto w-full max-w-6xl space-y-6">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.28em] text-zinc-500">{t('profile.title')}</p>
                  <h1 className="mt-1 text-2xl font-semibold text-[#0d141c] md:text-3xl">{heroGreeting}</h1>
                </div>
                <p className="text-sm text-zinc-600 md:text-base">{heroSubtitle}</p>
                <div className="flex flex-wrap items-center gap-3 text-xs font-medium uppercase tracking-[0.2em] text-zinc-500">
                  <span className="rounded-full bg-[#f6f7fb] px-3 py-1 text-[#0d141c]">{ordersCountLabel}</span>
                  {profile.email ? (
                    <span className="rounded-full bg-[#f6f7fb] px-3 py-1 text-[#0d141c]">{profile.email}</span>
                  ) : null}
                  <span className="rounded-full bg-[#f6f7fb] px-3 py-1 text-[#0d141c]">ID: {profile.uid}</span>
                </div>
              </div>
            </div>
            <div className="grid w-full gap-4 sm:grid-cols-2 lg:max-w-md">
              <div className="rounded-2xl border border-zinc-200 bg-[#f6f7fb] px-4 py-3">
                <p className="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500">{t('profile.ordersPreview.heading')}</p>
                <p className="mt-2 text-lg font-semibold text-[#0d141c]">{totalOrders}</p>
                <Link
                  href="/user/orders"
              ...
              <div className="rounded-2xl border border-zinc-200 bg-[#f6f7fb] px-4 py-3">
                <p className="text-xs font-medium uppercase tracking-[0.2em] text-zinc-500">{t('profile.addresses.heading')}</p>
                <p className="mt-2 text-lg font-semibold text-[#0d141c]">{addressesCount}</p>
                <p className="text-xs text-zinc-500">
                  {defaultAddress
                    ? defaultAddress.name || defaultAddress.line1 || defaultAddress.city || t('profile.addresses.defaultBadge')
              ...
              <div className="sm:col-span-2 rounded-2xl border border-[#0d141c]/15 bg-[var(--color-primary-dark)] px-4 py-4 text-white shadow">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/60">Account status</p>
                ...
                {!user?.emailVerified ? (
                  ...
                  <p className="mt-3 text-xs text-white/70">{t('profile.verification.success')}</p>
                )}
                <button
                  type="button"
                  onClick={() => logout()}
        ...
        <section className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          ...
          >
            <div>
              <h2 className="text-lg font-semibold text-[#0d141c]">{t('profile.accountCard.heading')}</h2>
              <p className="text-sm text-zinc-600">{t('profile.accountCard.description')}</p>
            </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="rounded-3xl border border-zinc-200 bg-white/90 px-6 py-6 shadow-[0_18px_36px_rgba(15,23,42,0.08)] sm:px-8">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-[#0d141c]">{t('profile.favorites.heading')}</h2>
                  <p className="text-sm text-zinc-600">{t('profile.favorites.description')}</p>
                </div>
                <Link
                  href="/favorites"
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                ...
                >
                  ...
                  <span className="text-xs text-zinc-500 transition group-hover:text-[#0d141c]">{'>'}</span>
                </Link>
                <Link
                  href="/cart"
                  className="group flex items-center justify-between rounded-2xl border border-zinc-200 bg-[#f6f7fb] px-4 py-3 text-sm font-semibold text-[#0d141c] transition hover:border-[#0d141c]/40 hover:bg-white"
            ...
            <div className="rounded-3xl border border-zinc-200 bg-white/90 px-6 py-6 shadow-[0_18px_36px_rgba(15,23,42,0.08)] sm:px-8">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-[#0d141c]">{t('profile.ordersPreview.heading')}</h2>
                  <p className="text-sm text-zinc-600">{t('profile.hero.subtitle')}</p>
                </div>
                <Link
                  href="/user/orders"
              ...
              {recentOrders.length === 0 ? (
                <p className="mt-4 text-sm text-zinc-600">{t('profile.ordersPreview.empty')}</p>
              ) : (
                <div className="mt-4 space-y-3">
                  {recentOrders.map((order) => {
                    ...
                    const formattedDate = orderDate
                      ...
                      : '--'
                    const amountLabel = formatCurrency(order.amountTotal ?? null, order.currency ?? null) || '--'
                    const itemsLabel = t('orders.orderCard.items').replace('{count}', String(getItemsCount(order)))
                    return (
                      <Link
                        key={order.id}
                      ...
                      >
                        <div>
                          <p className="font-semibold">#{order.id.slice(0, 10).toUpperCase()}</p>
                          <p className="text-xs text-zinc-500">
                            {t('orders.orderCard.placedOn').replace('{date}', formattedDate)}
                          </p>
                        </div>
        ...
        <section className="rounded-3xl border border-zinc-200 bg-white/90 px-6 py-6 shadow-[0_18px_36px_rgba(15,23,42,0.08)] sm:px-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-[#0d141c]">{t('profile.addresses.heading')}</h2>
              <p className="text-sm text-zinc-600">{t('profile.addresses.description')}</p>
            </div>
            <button
              type="button"
              onClick={() => {
            ...
            >
              {showAddressForm || addrForm.id ? t('profile.addresses.form.cancel') : t('profile.addresses.add')}
            </button>
          </div>
          <div className="mt-5 space-y-3">
            {sortedAddresses.length === 0 ? (
              <p className="text-sm text-zinc-600">{t('profile.addresses.empty')}</p>
            ) : (
                ...
                >
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2 text-[#0d141c]">
                      <span className="font-semibold">{a.name || t('profile.addresses.form.name')}</span>
                      {a.isDefault ? (
          ...
          {(showAddressForm || addrForm.id) && (
            ...
            >
              <h3 className="md:col-span-2 text-base font-semibold text-[#0d141c]">
                {addrForm.id ? t('profile.addresses.form.headingEdit') : t('profile.addresses.form.headingAdd')}
              </h3>
              <label className="flex flex-col gap-1 text-sm font-medium text-zinc-600">
